...
void IntroduceControlFlowSSA::operator()(Block& _block)
{
	...
	util::iterateReplacing(
		...
		[&](Statement& _s) -> std::optional<std::vector<Statement>>
		{
			...
			{
				YulName newName = m_yulNameRepository.deriveName(toReassign);
...
void SSATransform::run(OptimiserStepContext& _context, Block& _ast)
{
	...
	IntroduceSSA{assignedVariables, typeInfo, _context.yulNameRepository}(_ast);
	IntroduceControlFlowSSA{assignedVariables, typeInfo, _context.yulNameRepository}(_ast);
