...
{
	auto const* memoryStoreFunction = _yulNameRepository.memoryStoreFunction(_yulNameRepository.predefined().defaultType);
...
FunctionCall generateMemoryLoad(YulNameRepository const& _yulNameRepository, langutil::DebugData::ConstPtr const& _debugData, LiteralValue const& _mpos)
{
	auto const* memoryLoadFunction = _yulNameRepository.memoryLoadFunction(_yulNameRepository.predefined().defaultType);
...
void StackToMemoryMover::operator()(FunctionDefinition& _functionDefinition)
{
	...
	if (_functionDefinition.returnVariables.size() == 1 && m_memoryOffsetTracker(_functionDefinition.returnVariables.front().name))
	{
		...
		YulName newFunctionName = m_context.yulNameRepository.deriveName(_functionDefinition.name);
		...
		for (TypedName const& _var: stackParameters)
			newArgumentNames[_var.name] = m_context.yulNameRepository.deriveName(_var.name);
...
void StackToMemoryMover::operator()(Block& _block)
{
	...
	auto rewriteAssignmentOrVariableDeclarationLeftHandSide = [this](
		...
		if (_stmt.value)
		{
			...
			if (m_context.yulNameRepository.isBuiltinName(functionCall->functionName.name))
		...
		for (auto&& [lhsVar, rhsSlot]: ranges::views::zip(_lhsVars, rhsMemorySlots))
		{
			...
			if (rhsSlot)
				rhs = std::make_unique<Expression>(generateMemoryLoad(m_context.yulNameRepository, debugData, *rhsSlot));
			else
			{
				YulName tempVarName = m_context.yulNameRepository.deriveName(lhsVar.name);
...
void StackToMemoryMover::visit(Expression& _expression)
{
	...
	if (Identifier* identifier = std::get_if<Identifier>(&_expression))
		if (auto offset = m_memoryOffsetTracker(identifier->name))
			_expression = generateMemoryLoad(m_context.yulNameRepository, identifier->debugData, *offset);
}
...
std::optional<LiteralValue> StackToMemoryMover::VariableMemoryOffsetTracker::operator()(YulName const& _variable) const
{
	if (m_memorySlots.count(_variable))
	{
		...
		auto const value = m_reservedMemory + 32 * (m_numRequiredSlots - slot - 1);
		return valueOfNumberLiteral(toCompactHexWithPrefix(value));
	}
	else
		return std::nullopt;
}
